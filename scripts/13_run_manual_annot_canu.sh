...
OUTPUTDIR=${WORKDIR}/te_manual_annot/canu

cd ${OUTPUTDIR}

# 1.
seqkit fx2tab -l -n ${WORKDIR}/polished_assemblies/canu/pilon.fasta | head -n 1 | cut -f 1 > ID.txt
seqkit grep -n -f ID.txt ${WORKDIR}/polished_assemblies/canu/pilon.fasta -o ${OUTPUTDIR}/long_contig.fa

seqkit fx2tab -l -n ${WORKDIR}/polished_assemblies/canu/pilon.fasta > ID.txt
...
# 6.
head -n 1 contig_windows_top50.fa | cut -f 1 > window.fa # should create a new file that selects one window 
makeblastdb -in ${WORKDIR}/polished_assemblies/canu/pilon.fasta -dbtype nucl -out blast_db/genome
blastn -query window.fa -db blast_db/genome -outfmt 6 -num_threads 10 -perc_identity 80 -qcov_hsp_perc 80 -max 1 > window.blastn

makeblastdb \
    -in ${WORKDIR}/polished_assemblies/canu/pilon.fasta \
    -dbtype nucl \
    -out blast_db/genome

# Just chose the first contig of the file
awk -v seq='tig00000001_pilon_sliding:52501-53000' \
...
# 8.
seqkit subseq \
    --bed window.bed ${WORKDIR}/polished_assemblies/canu/pilon.fasta \
    -u 2000 \
    -d 2000 \
    > window.bed.fa
