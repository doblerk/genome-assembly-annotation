...
#SBATCH --job-name=dupgene
#SBATCH --output=/data/users/kdobler/assembly_annotation_course/outputs/output_dupgen_flye_%j.o
#SBATCH --error=/data/users/kdobler/assembly_annotation_course/outputs/error_dupgen_flye_%j.e
...
OUTPUTDIR=${WORKDIR}/dupgen_outputs/flye
...
# Reformat some files
awk '{sub(/ID=/, "", $9); sub(/;.*/, "", $9); sub(/:.*/, "", $9); printf ("%s\t%s\t%s\t%s\n", $1, $9, $4, $5)}' ${WORKDIR}/maker_outputs/flye/pilon.all.gff | grep '\-RA' > Ath.gff
...
makeblastdb \
 -in ${WORKDIR}/maker_outputs/flye/pilon.all.maker.proteins.fasta \
 -dbtype prot \
 -out blast_db/Ath

# Align protein query sequences against the reference database
blastp \
 -query ${WORKDIR}/maker_outputs/flye/pilon.all.maker.proteins.fasta \
 -db blast_db/Ath \
 -evalue 1e-10 \
 -max_target_seqs 5 \
 -outfmt 6 \
...
seqkit grep -f ID.txt ${WORKDIR}/maker_outputs/flye/pilon.all.maker.transcripts.fasta -o Ler.wgd.genes.fa
