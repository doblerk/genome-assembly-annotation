...
#SBATCH --output=/data/users/kdobler/assembly_annotation_course/outputs/output_phylo_canu_%j.o
#SBATCH --error=/data/users/kdobler/assembly_annotation_course/outputs/error_phylo_canu_%j.e
...
INPUTDIR=${WORKDIR}/tesorter_outputs/canu
OUTPUTDIR=${WORKDIR}/phylogenetic_analysis/canu

cd ${OUTPUTDIR}

# Extract RT protein sequences ID for copia
more ${INPUTDIR}/parsed.fa.rexdb-plant.cls.pep | grep Ty1-RT | sed 's/>//'| sed 's/ .\+//' > copia/ID_ar.txt
more ${INPUTDIR}/Ler_canu_TEs.rexdb-plant.cls.pep | grep Ty1-RT | sed 's/>//'| sed 's/ .\+//' > copia/ID_ar.txt
more ${INPUTDIR}/brassicaceae.cls.pep | grep Ty1-RT | sed 's/>//' | sed 's/ .\+//' > copia/ID_brass.txt

# Extract RT protein sequences ID for gypsy
more ${INPUTDIR}/parsed.fa.rexdb-plant.cls.pep | grep Ty3-RT | sed 's/>//'| sed 's/ .\+//' > gypsy/ID_ar.txt
more ${INPUTDIR}/Ler_canu_TEs.rexdb-plant.cls.pep | grep Ty3-RT | sed 's/>//'| sed 's/ .\+//' > gypsy/ID_ar.txt
...
seqkit grep \
    -f copia/ID_ar.txt \
    ${INPUTDIR}/Ler_canu_TEs.rexdb-plant.cls.pep \
    -o copia/ar.fa
